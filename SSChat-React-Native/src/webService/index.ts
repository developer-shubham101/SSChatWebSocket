...
export class AppOperation {
    ...
    constructor(options){
        ...
        this.root_path = ``;
        this.base_media_url = this.configuration.base_media_url;

        this.customer = customer(this);
        console.log(new Error(JSON.stringify(options)));
    }
    ...
    send(url, method, params, data, type) {
        ...
        return new Promise((resolve, reject) => {
            ...
            let bodyData;
            if (data instanceof FormData) {//asuming value is already set
                bodyData = data;
            ...
            fetch(uri, {method, headers, body: bodyData/* JSON.stringify(data) */})
                .then((response) => {
                    ...
                    if (response.ok) {
                        ...
                        }).catch(errorResponse => Promise.reject({code: status, data: errorResponse}));
                    }
                    // Possible 401 or other network error
                    return response.text().then(errorResponse => Promise.reject({code: status, data: errorResponse}));
                })
                .catch((error) => {
    ...
    getErrorMessageForResponse(data) {
        // console.log("getErrorMessageForResponce", data);
    ...
    getMediaUrl(file) {
        return `${this.base_media_url}${file}`;
    }

}

export const appOperation = new AppOperation(appOptions);
